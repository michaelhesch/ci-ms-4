...
def cache_checkout_data(request):
    try:
        ...
        # Modify payment intent with additional metadata
            ...
            if form.is_valid():
                ...
                for sku, item_data in cart.items():
                    try:
                        ...
                        liffey_amount = round(
                            Decimal(product.price) * Decimal(
                                item_data) * Decimal(0.05), 2)
                        vendor_amount = round(
                            Decimal(product.price) * Decimal(
                                item_data) * Decimal(0.95), 2)
                ...
                # Set order date, ordered status to True and save
                current_cart = cart_contents(self.request)
                cart_total = current_cart.get('grand_total')
